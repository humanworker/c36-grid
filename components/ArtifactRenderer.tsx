...
import { Wrench, Zap, Cherry, Package } from 'lucide-react';
        ...
        return (
            <div className={`${className} bg-green-900/20 flex flex-col items-center justify-center p-2 rounded border border-green-900/50`}>
                <div className="bg-green-500 p-3 rounded-full mb-1">
                   {/* Differentiate map food (Fruit/Cherry), Sandwich, vs Pantry items (Package/Can) */}
                   {data.name === 'Fruit' ? <Cherry size={24} className="text-white"/> : 
                    data.icon === 'SANDWICH' ? (
                        // Custom Sandwich SVG
